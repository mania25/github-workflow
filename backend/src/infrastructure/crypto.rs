...
impl PQCrypto {
    pub fn new() -> Result<Self> {
        ...
        let server_key_bytes = {
            let mut key = [0u8; 32];
            rng.fill(&mut key).map_err(|_| anyhow!("Failed to generate server key"))?;
            key
        };
    ...
    pub fn generate_session_key(&self, client_public_key: &[u8]) -> Result<[u8; 32]> {
        ...
        self.rng.fill(&mut session_key_bytes)
    ...
    pub fn encrypt(&self, data: &str) -> Result<String> {
        ...
        self.server_key.seal_in_place_append_tag(nonce, aead::Aad::from(b"todo-app"), &mut in_out)
    ...
    pub fn decrypt(&self, encrypted_data: &str) -> Result<String> {
        ...
        let plaintext = self.server_key.open_in_place(nonce, aead::Aad::from(b"todo-app"), &mut in_out)
    ...
    pub fn decrypt_with_session(&self, encrypted_data: &str, client_public_key: &[u8]) -> Result<String> {
        let session_keys = self.session_keys.lock()
            .map_err(|_| anyhow!("Failed to acquire session keys lock"))?;
        
        let session_key = session_keys.get(client_public_key)
        ...
        let mut in_out = ciphertext.to_vec();
        let plaintext = session_key.open_in_place(nonce, aead::Aad::from(b"todo-app"), &mut in_out)
            .map_err(|_| anyhow!("Failed to decrypt data"))?;
